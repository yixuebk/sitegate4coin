(function($){
    ...
    function updateGenFrom() {
        if( gen_from == 'pass' ) {
            ...
            $("#bip32_pub_source_key").attr('readonly', true);
            $("#gen_from_msg").html("Add your strings to passphrase. It will be hashed using 50,000 rounds of HMAC-SHA256");
        } else {
    ...
    function updatePassphraseHash() {
        ...
        $("#bip32_pub_source_key").val(gen_bip32.extended_public_key_string("base58"));
        updateSourceKey();
    }
        ...
        if( gen_from == 'pass' && bip32_source_key === null ) {
            ...
            if( COINS[coin].prefix != key_coin.prefix ) { // key is changing to another realm..
                ...
                if( is_public ) {
                    ...
                    $("#bip32_pub_source_key").val(bip32_source_key.extended_public_key_string("base58"));
                } else if( is_private ) {
    ...
    function start_hash_worker(passphrase) {
        if( hash_worker === null ) {
            hash_worker = new Worker("/site_media/static/js/hash_worker.js");
    ...
    $(document).ready( function() {
        ...
        var length = 64 
        var randomStr = '';
        for (var i = length; i > 0; --i) randomStr += chars[Math.round(Math.random() * (chars.length - 1))];

        //$("#bip32_source_passphrase").val("crazy horse battery staple");
        $("#bip32_source_passphrase").val(randomStr);
        ...
        bip32_passphrase_hash = Crypto.util.bytesToHex(Crypto.SHA256(randomStr, { asBytes: true }));
        //start_hash_worker(randomStr)
        updateDerivationPath();
